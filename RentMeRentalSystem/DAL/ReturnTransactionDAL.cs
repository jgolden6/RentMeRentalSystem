...
namespace RentMeRentalSystem.DAL
{
    class ReturnTransactionDAL
    {
         ...
         public List<Transaction> RetrieveAllReturnTransactions()
        {
            using (var conn = new MySqlConnection(Connection.connectionString))
            {
                ...
                cmd.Parameters["@type"].Value = TransactionType.Return.ToString().ToLower();
         ...
         public List<Transaction> RetrieveCustomerReturnTransactions(string customerId)
         {
             using (var conn = new MySqlConnection(Connection.connectionString))
             {
                 conn.Open();
                 ...
                 using var reader = cmd.ExecuteReader();
                 var retrieved = this.helpRetrieveTransactions(reader, TransactionType.Return);
                 return retrieved;
             }
         }
        ...
        private List<Transaction> helpRetrieveTransactions(MySqlDataReader reader, TransactionType type)
        {
            ...
            var transactionDateOrdinal = reader.GetOrdinal("returnDate");
            while (reader.Read())
            {
         ...
         public bool CreateReturnTransaction(JsonArray rentalBase, JsonArray rentalItems)
        {
            var success = this.createReturnTransaction(rentalBase, rentalItems);
            return success;
        }

        private bool createReturnTransaction(JsonArray rentalBase, JsonArray rentalItems)
        {
        ...
        public void UpdateFurnitureQuantities()
        {
            using (var conn = new MySqlConnection(Connection.connectionString))
            {
                conn.Open();
                using (var cmd =
                    new MySqlCommand("call update_furniture_quantities_after_return()", conn))
                {
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        public double CalculateReturnTransactionCost(JsonArray items)
        {
            var cost = 0.0m;
